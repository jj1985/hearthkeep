...
func test_four_named_bosses_present() -> void:
    var bosses: Array = []
    for k in _enemies().keys():
        if bool(_enemies()[k].get("boss", false)):
            bosses.append(k)
    assert_eq(bosses.size(), 4)
    for k in ["boss_warchief", "boss_dragon", "boss_aethyrnax", "boss_ourzhal"]:
        assert_true(bosses.has(k), "missing boss %s" % k)
